#!/usr/bin/env node
/**
 * Capture cross merge animation as a GIF using Playwright VIDEO recording.
 *
 * Cross pattern (3×3 anchored at grid (1,1)):
 *   Any  Own  Any       (2,1)=Own
 *   Own  Own  Own       (1,2)=Own  (2,2)=Own✓  (3,2)=Own
 *   Any  Own  Any       (2,3)=Own
 *
 * Click plan: place (2,1), (1,2), (3,2), (2,3). Last triggers merge.
...
const OUTPUT_GIF = path.join(ROOT, 'html/gifs/cross-merge.gif');
const GIF_SIZE = 267;
const CROP_BORDER_PX = 2;

const SETUP_SETTLE_MS = 4000;
const INITIAL_HOLD_MS = 100;
const AFTER_PLACE_MS = 500;        // game needs time to process each placement
...
async function main() {
  ...
  try {
    ...
    // Phase 2-4: Place (2,1), (1,2), (3,2)
    for (const [x, y] of [[2, 1], [1, 2], [3, 2]]) {
      const c = cellCenter(x, y);
      await gameClick(c.x, c.y);
      await delay(AFTER_PLACE_MS);
    }

    // Phase 5: Place (2,3) — bottom → triggers merge
    const cMerge = cellCenter(2, 3);
