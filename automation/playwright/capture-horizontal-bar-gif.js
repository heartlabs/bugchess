 ...
 *
 * HorizontalBar pattern (3×3 anchored at grid (1,1)):
 *   Free Free Free
 *   Own  Own  Own      (1,2)=Own  (2,2)=Own✓  (3,2)=Own
 *   Free Free Free
 *
 * Click plan: place (1,2), then (3,2). Last triggers merge.
 * Crop: 3×3 centered on (2,2), origin cell (1,1).
 *
 * Uses Canvas2D scaling: game renders to 1800×1600, scaled to viewport.
 */
const fs = require('fs');
...
const AFTER_MERGE_PLACE_MS = 5000; // final placement triggers merge
...
async function main() {
  ...
  try {
    ...
    // Place (1,2) — need delay for game to process
    const c12 = cellCenter(1, 2);
    await gameClick(c12.x, c12.y);
    await delay(AFTER_PLACE_MS);

    // Place (3,2) — triggers merge
    const c32 = cellCenter(3, 2);
    await gameClick(c32.x, c32.y);
    await page.mouse.move(5, 5);
