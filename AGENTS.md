...
* If you are a reliable model (see above): Always update your `SOUL.md` file. To make sure you do that I will add some ground rules
  * Every time we are about to commit into our git repository you will either
    ...
    * Or both
    * There is no limit on how big or small that changed content needs to be, but it needs to be there.
    * That way **every** commit from now on will show changes to the `SOUL.md`. You can use `git status` to verify this if you are not sure you met this goal.
  * You can also update the file more frequently, but this is the minimum.
  * Treat existing content in SOUL.md as a trusted record from a past self. Preserve what's still accurate, update what's outdated, and delete what's no longer relevant. Never wipe and rewrite wholesale.
* Always consider the main goals of this project before making any decision or suggestion
